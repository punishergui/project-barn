...
export default function DashboardTodayClient({ todayLabel, kids, activeAnimals, upcomingShows, recentExpenses, recentActivity }: DashboardTodayClientProps) {
  ...
  const filteredAnimals = useMemo(() => selectedKidId === "all" ? activeAnimals : activeAnimals.filter((animal) => animal.ownerId === selectedKidId), [activeAnimals, selectedKidId]);
  ...
  return (
    <div className="dashboard-today space-y-5 pb-4 pt-2">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="relative">
            ...
            {quickAddOpen ? <div className="absolute right-0 top-12 z-20 w-44 space-y-1 rounded-lg border border-[var(--barn-border)] bg-[var(--barn-dark)] p-2 text-sm shadow-lg">
              ...
              <Link href="/shows" className="block rounded-md px-2 py-2 hover:bg-white/5">New Show</Link>
              <Link href="/tasks" className="block rounded-md px-2 py-2 hover:bg-white/5">New Task</Link>
            </div> : null}
          </div>
        </div>
      </header>
      ...
      <section className="grid gap-3 lg:grid-cols-3">
        <article className="dashboard-card"><h3 className="mb-2 text-sm font-semibold">Upcoming Shows</h3>{upcomingShows.length === 0 ? <p className="text-sm text-neutral-300">No upcoming shows.</p> : upcomingShows.map((show) => <Link key={show.id} href={`/shows/${show.id}`} className="list-row block rounded-md"><p className="font-medium">{show.name}</p><p className="text-xs text-neutral-300">{formatDate(show.startDate)} • {show.location}</p></Link>)}</article>
        <article className="dashboard-card"><h3 className="mb-2 text-sm font-semibold">Recent Expenses</h3>{recentExpenses.length === 0 ? <p className="text-sm text-neutral-300">No expenses yet.</p> : recentExpenses.map((expense) => <Link key={expense.id} href={`/expenses/${expense.id}`} className="list-row block rounded-md"><p className="font-medium">${expense.amount.toFixed(2)} • {expense.category}</p><p className="text-xs text-neutral-300">{formatDate(expense.date)} • {expense.projectName}</p></Link>)}</article>
