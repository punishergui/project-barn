...
export default function ProjectRemindersPage() {
  ...
  return (
    <div className="w-full space-y-4 px-4 pb-6">
      ...
      <form className="rounded-2xl bg-card border border-border shadow-sm p-4 grid gap-2 text-sm" onSubmit={(event) => createReminder(event).catch(() => undefined)}>
        <select name="type" className="rounded bg-background p-2">{reminderTypes.map((type) => <option key={type} value={type}>{type.replace("_", " ")}</option>)}</select>
        <input name="time_of_day" type="time" className="rounded bg-background p-2" />
        <input name="frequency" placeholder="daily, weekdays, custom" className="rounded bg-background p-2" />
        <textarea name="notes" placeholder="Reminder message" className="rounded bg-background p-2" />
        <label className="flex items-center gap-2"><input name="parent_locked" type="checkbox" /> Parent locked</label>
        <button className="rounded bg-primary px-3 py-2 text-sm">Add reminder</button>
      </form>

      {rows.length === 0 ? <p className="text-sm text-muted-foreground text-sm text-muted-foreground">No reminders yet.</p> : rows.map((item) => {
        const blocked = item.parent_locked && auth?.role !== "parent";
        return (
          <article key={item.id} className="rounded-2xl bg-card border border-border shadow-sm p-4 space-y-2 text-sm">
            <p className="font-semibold">{item.type.replace("_", " ")}{item.parent_locked ? " 🔒" : ""}</p>
            <p className="text-xs text-muted-foreground">{item.time_of_day || "Any time"} • {item.frequency || "No frequency"}</p>
            {item.notes ? <p>{item.notes}</p> : null}
            <div className="flex gap-2">
              <button disabled={blocked} onClick={() => toggle(item).catch(() => undefined)} className="rounded bg-background px-3 py-2 text-xs disabled:opacity-50">{item.enabled ? "Disable" : "Enable"}</button>
