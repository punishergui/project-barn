...
export default function FeedInventoryPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch("/api/feed-inventory", { credentials: "include", cache: "no-store" });
  ...
  return (
    <div className="flex flex-col gap-3 pb-28">
      {items.map((item) => (
        <div key={item.id} className="rounded-2xl bg-card border border-border shadow-sm p-4">
          ...
          <div className="mt-3 flex items-center gap-3">
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full bg-secondary text-foreground text-lg font-bold"
            >
              −
            </button>
