...
export function ShowsMediaCard({ item }: { item: MediaItem }) {
  ...
  return (
    <a href={mediaUrl} className="relative space-y-1 rounded bg-background p-2">
      {mediaType === "video" ? (
      ...
      <p className="truncate text-xs text-muted-foreground">{item.caption ?? item.file_name}</p>
      <p className="text-[11px] text-muted-foreground">{item.show_name || (item.show_id ? "Show" : "Project")}{item.placing_value ? ` • ${item.placing_value}` : ""}</p>
    </a>
  );
}
